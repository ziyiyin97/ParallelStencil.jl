...
@parallel function compute_dτ!(dτVx::Data.Array, dτVy::Data.Array, dτPt::Data.Array, μs::Data.Array, min_dxy2::Data.Number, β_n::Data.Number, Vsc::Data.Number, Ptsc::Data.Number, nx::Int, ny::Int)
    ...
    @all(dτVy) = min_dxy2/@av_yi(μs)/(1.0+β_n)/4.1/Vsc
    @all(dτPt) = 4.1*@all(μs)*(1.0+β_n)/max(nx,ny)/Ptsc
    return
end
...
@parallel function compute_P_τ!(Pt::Data.Array, Pf::Data.Array, τxx::Data.Array, τyy::Data.Array, σxy::Data.Array, RPt::Data.Array, RPf::Data.Array, dτPf::Data.Array, Vx::Data.Array, Vy::Data.Array, ∇V::Data.Array, dτPt::Data.Array, μs::Data.Array, β_n::Data.Number, dx::Data.Number, dy::Data.Number)
    ...
    @all(σxy) = 2.0*@av(μs)*(0.5*( @d_yi(Vx)/dy + @d_xi(Vy)/dx ))
    return
end
...
@parallel function compute_update!(Vx::Data.Array, Vy::Data.Array, qDx::Data.Array, qDy::Data.Array, Phi::Data.Array, dVxdτ::Data.Array, dVydτ::Data.Array, K_muf::Data.Array, Pf::Data.Array, Phi_o::Data.Array, ∇V::Data.Array, ∇V_o::Data.Array, dτVx::Data.Array, dτVy::Data.Array, ρfg::Data.Number, ρgBG::Data.Array, CN::Data.Number, dt::Data.Number, dx::Data.Number, dy::Data.Number)
    # overload
...
@views function HydroMech2D()
    ...
    lx       = 3500.0            # domain size x
    ly       = ra*lx           # domain size y
    ϕA       = 0.15            # amplitude of initial porosity perturbation       
    ...
    #dτV      = min_dxy2/maximum(μs)/(1.0+β_n)/4.1/Vsc     # PT time step for velocity
    dτVx = @zeros(nx-1,ny-2)
    dτVy = @zeros(nx-2,ny-1)
    dτPt = @zeros(nx  ,ny  )
    #dτPt     = 4.1*maximum(μs)*(1.0+β_n)/max(nx,ny)/Ptsc
    dampX    = 1.0-Vdmp/nx
    dampY    = 1.0-Vdmp/ny
    # Array allocations
    Phi_o    = @zeros(nx  ,ny  )
    ...
    qDy      =   zeros(nx  ,ny+1)
    Phi      = deepcopy(ϕ0)
    Phi[55:73,105:108] .= Phi[55:73,105:108] .+ ϕA
    EtaC     = μs./Phi.*η2μs.*0.0 .+ 1.0
    K_muf    = k_μf0.*(Phi./ϕ0).*0.0 .+ 1.0
    ϕ0bc     = mean.(Phi[:,end])
    qDy[:,[1,end]] .= (ρsg.-ρfg).*(1.0.-ϕ0bc).*k_μf0.*(ϕ0bc[:,[1,end]]./ϕ0[:,[1,end]]).^nperm
    Phi      = Data.Array(Phi)
    ...
    while t<t_tot
        ...
        while err > ε && iter <= iterMax
            ...
            @parallel compute_dτ!(dτVx, dτVy, dτPt, μs, min_dxy2, β_n, Vsc, Ptsc, nx, ny)
            ...
            @parallel compute_update!(Vx, Vy, qDx, qDy, Phi, dVxdτ, dVydτ, K_muf, Pf, Phi_o, ∇V, ∇V_o, dτVx, dτVy, ρfg, ρgBG, CN, dt, dx, dy)
